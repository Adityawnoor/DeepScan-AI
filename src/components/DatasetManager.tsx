  ...
  const syncCloudToPCVault = async () => {
    ...
    try {
      ...
      toast({ 
        ...
        description: `Cryptographic snapshot saved to ${vaultHandle.name}. Cloud state is now offline-ready.` 
      })
    } catch (e: any) {
      console.error(e)
      ...
      if (e.code === 'permission-denied') {
        errorEmitter.emit('permission-error', new FirestorePermissionError({ path: 'root_sync', operation: 'list' }))
      }
    } finally {
      setIsSyncing(false)
      setSyncStep("")
    ...
    <div className="space-y-6">
        ...
        >
          <div className="space-y-1">
             ...
             <p className="text-[8px] font-bold uppercase text-muted-foreground/60">
               {vaultHandle ? "FORENSIC MIRROR ACTIVE" : "PHYSICAL DATABASE UNLINKED"}
             </p>
          </div>
        </Card>
      </div>
            ...
            <CardContent className="pt-8">
              ...
              <div className="space-y-4">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                  <ShieldCheck className="w-4 h-4" /> Learned Neural Signatures
                </h4>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {(!datasets || datasets.length === 0) ? (
                        <TableRow><TableCell colSpan={4} className="text-center py-12 text-[10px] font-bold text-muted-foreground uppercase opacity-30">No forensic signatures learned yet.</TableCell></TableRow>
                      ) : (
                        datasets.map((item) => (
          ...
          <CardContent className="space-y-6 pt-6">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input placeholder="e.g., StableVideo-v2, GAN-LipSync..." className="text-xs h-12 rounded-xl bg-background/50 font-bold uppercase" value={modelSignature} onChange={(e) => setModelSignature(e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">2. Forensic Notes</Label>
                <Textarea placeholder="Describe artifact patterns (e.g. Ocular jitter, spectral gaps)..." className="text-xs min-h-[120px] rounded-xl bg-background/50" value={datasetNotes} onChange={(e) => setDatasetNotes(e.target.value)} />
              </div>
              <div className="space-y-2">
      ...
      <Dialog open={showBrainViewer} onOpenChange={setShowBrainViewer}>
        <DialogContent className="max-w-2xl rounded-3xl border-2 border-primary/20 volumetric-shadow overflow-hidden p-0">
          <DialogHeader className="p-8 pb-0"><DialogTitle className="flex items-center gap-3 text-2xl font-black uppercase tracking-tighter text-primary"><BrainCircuit className="w-8 h-8" /> Pattern Memory HUD</DialogTitle></DialogHeader>
          ...
          <DialogFooter className="p-8 pt-0"><Button onClick={() => setShowBrainViewer(false)} className="rounded-2xl font-black uppercase tracking-widest w-full h-14">Close Memory Viewer</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  )
