...
export function DatasetManager({ knowledgeCount, onVaultChange, vaultHandle, vaultPermissionStatus }: DatasetManagerProps) {
  ...
  const exportFullNeuralBackup = async () => {
    ...
    try {
      const fileName = `FULL_NEURAL_BACKUP_${new Date().toISOString().split('T')[0]}.json`
      const fileHandle = await vaultHandle.getFileHandle(fileName, { create: true })
      const writable = await (fileHandle as any).createWritable()
      const backup = {
        timestamp: new Date().toISOString(),
        totalFacts: knowledgeCount,
        accuracy: currentAccuracy,
        datasets: datasets,
        scans: scans.filter(s => s.userFeedback !== undefined)
      }
      await writable.write(JSON.stringify(backup, null, 2))
      await writable.close()
      toast({ title: "Memory Exported", description: "Complete Neural Knowledge mirrored to physical vault." })
    } catch (e: any) {
  ...
  const handleTrainingFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !datasetNotes.trim()) {
      toast({ variant: "destructive", title: "Missing Data", description: "Notes and file are required." })
      return
    }
    try {
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="bg-primary/5 border-primary/20 border-dashed border-2 flex items-center justify-center p-6 text-center cursor-pointer rounded-xl hover:bg-primary/10 transition-all hover-glow" onClick={() => setShowBrainViewer(true)}>
          <div className="space-y-1">
             ...
             <p className="text-[9px] text-muted-foreground">{knowledgeCount} Lessons Persisted</p>
          </div>
        </Card>
        ...
        <Card className={cn("bg-muted border border-border shadow-none rounded-xl flex items-center justify-center p-6 text-center cursor-pointer transition-all", vaultHandle ? "border-primary/50 bg-primary/5" : "hover:bg-muted/80")} onClick={handleConnectLocalPC}>
          <div className="space-y-1">
             ...
             <p className="text-[8px] font-bold uppercase text-muted-foreground/60">Shutdown Protection: {vaultHandle ? "READY" : "UNLINKED"}</p>
          </div>
        </Card>
      </div>
           ...
           <Card className="shadow-none border border-border rounded-xl overflow-hidden volumetric-shadow">
            <CardHeader className="bg-muted/30 pb-4 flex flex-row items-center justify-between">
               <CardTitle className="text-lg flex items-center gap-2 font-black uppercase tracking-tighter">
                  <Activity className="w-5 h-5 text-primary" /> Forensic Intelligence Mirror
                </CardTitle>
                <Button variant="outline" size="sm" className="rounded-xl font-black text-[10px] uppercase gap-2" onClick={exportFullNeuralBackup}>
                  <Save className="w-3.5 h-3.5" /> Full Neural Backup
                </Button>
            </CardHeader>
      ...
      <Dialog open={showBrainViewer} onOpenChange={setShowBrainViewer}>
        <DialogContent className="max-w-2xl rounded-2xl border-2 border-primary/20 volumetric-shadow">
          <DialogHeader><DialogTitle className="flex items-center gap-3 text-2xl font-black uppercase tracking-tighter text-primary"><BrainCircuit className="w-8 h-8" /> Neural Memory HUD</DialogTitle></DialogHeader>
